...
(() => {
  ...
  function translateAttributes(el){
    for(const attr of ['aria-label','placeholder','title']){
      ...
      const en=dynamicTranslate(raw.trim()); if(en!==raw.trim()) el.setAttribute(attr,en);
    }
  }
  function translateTree(root){
    ...
    let n;
    while((n=walker.nextNode())){ if(n.nodeType===Node.TEXT_NODE) replaceTextNode(n); else translateAttributes(n); }
  }
  ...
  function initialHomeGuard(){
    ...
    window.setTimeout(()=>{
      ...
      if(!publicHome && !science && typeof window.goHome==='function'){ try{ window.goHome(); }catch(_){} }
    },120);
  }
  ...
  function run(){ timer=0; translateVisiblePublic(); ensureQuestionnaireBoundaryNote(); }
  function schedule(){
    if(timer) clearTimeout(timer);
    timer=setTimeout(run,75);
  }
  function start(){
    addMissingRuntimeLabels(); bindAssessmentBoundary();
    const watch=document.getElementById('app')||document.body;
    new MutationObserver(schedule).observe(watch,{childList:true,subtree:true,attributes:true,attributeFilter:['aria-label','placeholder','title']});
    initialHomeGuard(); run();
  }

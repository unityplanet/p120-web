/* P-120 Founder Shell v2
   Shared navigation/theme/language shell for RU + EN Founder pages.
   Reuses the public Navigation Architecture v2 visual grammar while keeping
   dedicated Story routes path-safe from /creator/ and /en/creator/. */
(() => {
  'use strict';
  ...
  const copy=isEn?{
    ...
    together:'Together?',togetherNote:'Dyadic research layer',coming:'Coming',
    ...
    deeper:'Хотите глубже?',deeperNote:'Расширенный исследовательский набор · дополнительные исследования',
    together:'Мы вместе?',togetherNote:'Исследование пары · диадический слой',coming:'Готовится',
  ...
  function applyTheme(next,persist=true){
    const t=themes.includes(next)?next:'museum';
  ...
  function installTheme(){
    const slot=document.querySelector('[data-founder-theme-slot]');
    let initial='museum';try{initial=localStorage.getItem(themeKey)||'museum'}catch(_){}
    applyTheme(initial,false);
    if(slot)slot.innerHTML=themeMenu();
    applyTheme(initial,false);
    document.addEventListener('click',e=>{
      const b=e.target.closest('[data-set-theme]');if(!b)return;
      e.preventDefault();applyTheme(b.dataset.setTheme,true);
      b.closest('.header-theme-menu')?.removeAttribute('open');
    });
  }
  ...
  function installMega(){
    const slot=document.querySelector('[data-founder-explore-slot]');if(!slot)return;
    slot.innerHTML=mega();
    ...
    document.addEventListener('click',e=>{if(!e.target.closest('[data-founder-mega]'))closeMega()});
    document.addEventListener('keydown',e=>{if(e.key==='Escape')closeMega()});
  }

  function installFounderRoutes(){
    document.addEventListener('click',e=>{
      const b=e.target.closest('[data-founder-route]');if(!b)return;
      e.preventDefault();
      const r=b.dataset.founderRoute;
      if(r==='science')window.location.assign(paths.science);
      else if(r==='why')window.location.assign(paths.why);
      else if(r==='self')window.location.assign(paths.self);
    },true);
  }
  function start(){installTheme();installMega();installFounderRoutes();document.documentElement.dataset.founderShell='v2'}

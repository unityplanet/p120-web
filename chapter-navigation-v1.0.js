...
(() => {
  ...
  function waitForTarget(chapter,attempt=0){
    ...
    if (target) {
      smoothScroll(target);
      window.setTimeout(updateFromScroll, reducedMotion()?0:260);
      return;
    }
  ...
  function openChapter(chapter){
    closeDrawer();
    const home = homeRoot();
    if (home && targetElement(chapter)) {
      smoothScroll(targetElement(chapter));
      return;
    }
    try {
  ...
  function makeDesktopNav(){
    ...
    nav.innerHTML = `
      <div class="chapter-jump-nav-inner">
        <div class="chapter-jump-prefix">По главам</div>
        <div class="chapter-jump-track">
          ${chapters.map(ch => `<button type="button" class="chapter-jump-button" data-chapter-jump="${ch.id}" aria-label="${ch.index}. ${ch.label}"><span class="chapter-jump-index">${ch.index}</span><span>${ch.label}</span></button>`).join('')}
          <span class="chapter-jump-progress" aria-hidden="true"><i></i></span>
        </div>
      </div>`;
    nav.querySelectorAll('[data-chapter-jump]').forEach(btn => {
      const chapter = chapters.find(ch => ch.id === btn.dataset.chapterJump);
      if (chapter) btn.addEventListener('click',() => openChapter(chapter));
    });
    return nav;
  }
  ...
  function ensureDesktopNav(){
    ...
    if (!home) {
      ...
      document.documentElement.classList.remove('chapter-nav-visible');
      return false;
    }
  ...
  function makeMobileGroup(){
    ...
    group.innerHTML = `<span class="eyebrow">По главам</span>${chapters.map(ch => `
      <button type="button" class="mobile-menu-link chapter-mobile-link" data-chapter-mobile="${ch.id}">
        <div><div><span class="chapter-mobile-index">${ch.index}</span>${ch.label}</div><small>${ch.note}</small></div>
      </button>`).join('')}`;
    group.querySelectorAll('[data-chapter-mobile]').forEach(btn => {
      const chapter = chapters.find(ch => ch.id === btn.dataset.chapterMobile);
      if (chapter) btn.addEventListener('click',() => openChapter(chapter));
    });
    return group;
  }
  ...
  function sectionPositions(){
    ...
    return chapters.map(ch => {
      const el = targetElement(ch);
      if (!el) return null;
      return {chapter:ch,top:window.scrollY + el.getBoundingClientRect().top - offset};
  ...
  function setActive(id){
    ...
    document.querySelectorAll('[data-chapter-jump]').forEach(btn => {
      const active = btn.dataset.chapterJump === activeId;
      btn.classList.toggle('is-active',active);
      if (active) btn.setAttribute('aria-current','location'); else btn.removeAttribute('aria-current');
    });
    document.querySelectorAll('[data-chapter-mobile]').forEach(btn => {
      const active = btn.dataset.chapterMobile === activeId;
      btn.classList.toggle('is-active',active);
      if (active) btn.setAttribute('aria-current','location'); else btn.removeAttribute('aria-current');
    });
  }
  ...
  function updateFromScroll(){
    ...
    const shouldShow = window.innerWidth >= DESKTOP_MIN && window.scrollY > showThreshold;
    document.documentElement.classList.toggle('chapter-nav-visible',shouldShow);
    ...
    for (const item of positions) {
      if (item.top <= probe) current = item.chapter;
      else break;
    }
  ...
  function start(){
    ...
    run();
    document.documentElement.classList.add('chapter-navigation-ready');
  }

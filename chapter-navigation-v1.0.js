...
(() => {
  ...
  function scrollOffset(){
    if(window.innerWidth<DESKTOP_MIN) return topOffset()+14;
    const nav=document.querySelector('.chapter-jump-nav-inner');
    const navHeight=Math.max(48,Math.round(nav?.getBoundingClientRect().height||48));
    return topOffset()+navHeight+18;
  }
  ...
  function waitForTarget(chapter,attempt=0){
    ...
    if(target){setActive(chapter.id);smoothScroll(target);return;}

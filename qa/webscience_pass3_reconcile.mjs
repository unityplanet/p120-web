...
const MAX_VISUAL_DELTA_PERCENT=0.15;
const scenarios=[
...
async function canonicalScienceDom(page){return page.locator('.science-page').evaluate(el=>{const clone=el.cloneNode(true);clone.querySelectorAll('[style]').forEach(n=>{if(!(n.getAttribute('style')||'').trim())n.removeAttribute('style');});return clone.outerHTML;});}
...
function visualDelta(beforeBuffer,afterBuffer){const a=PNG.sync.read(beforeBuffer),b=PNG.sync.read(afterBuffer);if(a.width!==b.width||a.height!==b.height)return{ratio:100,pixels:a.width*a.height,detail:`dimension mismatch ${a.width}x${a.height} vs ${b.width}x${b.height}`};const diff=new PNG({width:a.width,height:a.height});const pixels=pixelmatch(a.data,b.data,diff.data,a.width,a.height,{threshold:0.1,includeAA:false});return{ratio:(pixels/(a.width*a.height))*100,pixels,detail:`${pixels}px / ${a.width}x${a.height}`};}
...
for(const s of scenarios){
  ...
  const beforeDom=await canonicalScienceDom(page);const beforePng=await page.locator('.science-page').screenshot();
  ...
  const afterDom=await canonicalScienceDom(page);const afterPng=await page.locator('.science-page').screenshot();const mutations=await page.evaluate(()=>{window.__P120_PASS3_OBSERVER?.disconnect();return window.__P120_PASS3_MUTATIONS||[];});const status=await page.evaluate(()=>window.P120ScienceAtlas?.status||null);
  ...
  const domSame=beforeDom===afterDom;const diff=domSame?null:firstDiff(beforeDom,afterDom);check(domSame,`${s.locale}-${s.width} canonical .science-page DOM unchanged`,domSame?'':JSON.stringify({diff,mutations:mutations.slice(0,12)}));
  const vd=visualDelta(beforePng,afterPng);check(vd.ratio<=MAX_VISUAL_DELTA_PERCENT,`${s.locale}-${s.width} visual delta <= ${MAX_VISUAL_DELTA_PERCENT}%`,`${vd.ratio.toFixed(6)}% · ${vd.detail}`);
...
await browser.close();const report={document:'WEB-SCIENCE EXT PASS 3 Reconciliation QA',generated_at:new Date().toISOString(),expected_core_sha:EXPECTED_SHA,max_visual_delta_percent:MAX_VISUAL_DELTA_PERCENT,checks:results.length,failures:fail.length,results,fail};fs.writeFileSync(path.join(OUT,'PASS3_RECONCILE_QA.json'),JSON.stringify(report,null,2));fs.writeFileSync(path.join(OUT,'PASS3_RECONCILE_QA.txt'),results.map(x=>`${x.status} ${x.name}${x.detail?` :: ${x.detail}`:''}`).join('\n')+`\n\n${fail.length?'FAIL':'PASS'} failures=${fail.length}\n`);console.log(`WEB-SCIENCE PASS3 RECONCILE checks=${results.length} failures=${fail.length}`);if(fail.length)process.exit(1);
